...
use crate::{StateIndex, TransitionIndex, ENFA, DFA};
...
impl<S, T> From<ENFA<S, T>> for NFA<S, T> {
    fn from(_enfa: ENFA<S, T>) -> NFA<S, T> {
        panic!("Not implemented")
    }
}

impl<S, T> From<DFA<S, T>> for NFA<S, T> {
    fn from(_dfa: DFA<S, T>) -> NFA<S, T> {
        panic!("Not implemented")
    }
}
...
mod tests {
    ...
    fn assert_eq<S: Clone + Debug + Eq + Hash, T: Clone + Debug + Eq + Hash>(expected: Expected<S, T>, actual: NFA<S, T>) {
    ...
    fn test_3() {
        ...
        let mut actual = NFA::new(oset![0, 1, 2, 3, 4]);
    ...
    fn test_4() {
        ...
        let mut actual = NFA::new(oset![0, 2]);
    ...
    fn test_5() {
        ...
        let mut actual = NFA::new(oset![0, 1, 2]);
    ...
    fn test_6() {
        let expected = Expected {
            initial: oset![0, 2, 4],
            transitions: set![
                (oset![0, 2, 4], 'a', oset![1, 3]),
                (oset![0, 2, 4], 'a', oset![1, 5])
            ],
            finals: set![oset![1, 3], oset![1, 5]]
        };
        let mut actual = NFA::new(oset![0, 2, 4]);
